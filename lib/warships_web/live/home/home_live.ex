defmodule WarshipsWeb.Home.HomeLive do
  ...
  def mount(_params, _session, socket) do
    ...
    case user_token do
      ...
      _ ->
        ...
        if Map.has_key?(socket.assigns.flash, "error") ||
             Map.has_key?(socket.assigns.flash, "info") do
          clean_flash()
        end
  ...
  def handle_event("req_player_challenges", params, socket) do
    if params["value"] == "true" do
      {:noreply, assign(socket, :req_player_challenges, true)}
    else
      {:noreply, assign(socket, :req_player_challenges, false)}
    end
  end
  ...
  def handle_event("save", %{"room_form" => params}, socket) do
    if String.length(params["room_name"]) < 3 do
      ...
      case resp do
        :ok ->
          if string_to_bool(params["with_challenge"]) do
            GameStore.toggle_challenge(params["room_name"])
          end
  ...
  def handle_event("sort", %{"sort" => order, "value" => ""}, socket) do
    {:noreply,
     socket
     |> assign(:sort, String.to_atom(order))
     |> assign(:rooms_data, Enum.sort(socket.assigns.rooms_data, String.to_atom(order)))}
  end
    ...
    case msg.event do
      ...
      "player_added" ->
        ...
        case target_room do
          ...
          _ ->
            target_room_u =
              Enum.at(target_room, 0)
              |> Map.replace(:players, msg.payload.player_count)
              |> Map.replace(:has_cpu?, GameStore.has_player_cpu?(msg.payload.room))

            rooms_data_u =
              Enum.map(rooms_data, fn x ->
                if x.room != msg.payload.room, do: x, else: target_room_u
              end)
      ...
      "redirect_after_challenge_accepted" ->
        ...
        if player == socket.assigns.nickname do
          {:noreply,
           ...
           |> push_navigate(to: ~p"/rooms/#{room_name}")}
        else
          {:noreply, socket}
        end
  ...
  defp get_rooms_data() do
    ...
    rooms =
      Enum.map(
        Enum.filter(running_rooms_, fn x -> hd(elem(x, 3)) == Warships.GameStore end),
        fn y -> elem(y, 0) end
      )
    ...
    Enum.sort(
      Enum.map(rooms, fn x ->
        %{
          :room => extract_name(x),
          :players => GameStore.get_player_count(extract_name(x)),
          :protected? => RoomStore.room_protected?(extract_name(x)),
          :has_cpu? => GameStore.has_player_cpu?(extract_name(x)),
          :challenge_allowed? => GameStore.has_challenge_enabled?(extract_name(x))
        }
      end)
    )
  end
