defmodule Warships.CPUPlayer do
  ...
  def start_link(game_name, cpu_name) do
    GenServer.start_link(__MODULE__, %{:game_name => game_name, :cpu_name => cpu_name},
      name: AppRegistry.using_via("CPU_player: " <> game_name)
    )
  end
    ...
    if turn == state.cpu_name do
      case check_for_hits(state.game) do
          ...
          case init_hit do
            ...
            init_hit ->
              ...
              shoot(
                state,
                init_hit,
                hd(CoordsGenStore.get_dir(state.game <> " CPU_coords")),
                false
              )
        ...
        {_coords, nil} ->
          shoot(
            state,
            [CoordsGenStore.get_init_hit(state.game <> " CPU_coords")],
            get_last_tested_dir_and_swap(state.game),
            false
          )
  ...
  defp shoot(state, coords, dir, from_head?) do
    case dir do
      0 ->
        ...
        new_coord =
          {<<hd(String.to_charlist(elem(get_from_list(coords, from_head?), 0))) - 1::utf8>>,
           elem(get_from_list(coords, from_head?), 1)}
      ...
      1 ->
        # {x, y + 1}
        new_coord =
          {<<hd(String.to_charlist(elem(get_from_list(coords, from_head?), 0)))::utf8>>,
           Integer.to_string(String.to_integer(elem(get_from_list(coords, from_head?), 1)) + 1)}
