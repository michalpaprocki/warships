defmodule Warships.GameStore do
  ...
  def init(init_arg) do
    ...
    {:ok, %{:game => init_arg, :turn => "unset", :state => :awaiting_players, :winner => "", :players => %{}, :rematch =>%{:challenger=> :none, :request=> false}, :new_challenger => nil}}
  end
      ...
      true ->
        case length(Map.keys(state.players)) do
          2 ->
            ...
            cond do
               Enum.member?(Enum.map(Map.to_list(state.players), fn x -> elem(x, 1).is_human? == false end), true) && state.new_challenger == nil ->
          ...
          _ ->
            ...
            if length(Map.keys(new_state_.players)) == 2 do
              updated_new_state_ = new_state_|> Map.replace(:state, :prep) |> Map.replace(:new_challenger, nil)
  ...
  def handle_call({:remove_player, params}, _from, state) do
        case remove_a_player(state, params.name) do
          {:non_empty, new_state} ->
            {:reply, :OK, new_state}
          {:empty, new_state} ->
            {:reply, :OK, new_state |> Map.replace(:state, :awaiting_players)|> Map.replace(:new_challenger, nil)}
        end
  end
  def handle_call({:toggle_ready, params}, _from, state) do
    ...
    cond do
      elem(Enum.at(new_state.players, 0), 1).ready && elem(Enum.at(new_state.players, 1), 1).ready ->
        ...
        if elem(Enum.at(new_state.players, 0), 1).is_human? == false || elem(Enum.at(new_state.players, 1), 1).is_human? == false do
  ...
  def handle_call({:has_cpu}, _from, state) do
     case Enum.member?(Enum.map(Map.to_list(state.players), fn x -> elem(x, 1).is_human? == false end), true) do
      false ->
  ...
  def handle_call({:accept_challenge}, _from, state) do
    ...
    WarshipsWeb.Endpoint.broadcast("player_changes", "redirect_after_challenge_accepted", {state.game, state.new_challenger})
  ...
  defp reset_game(state, game_state) do
    ...
    new_state = %{:game => state.game, :turn => Enum.at(Enum.filter(players, fn x-> x != state.winner end), 0), :state => game_state, :winner => "", :players => players_map , :rematch =>%{:challenger=> :none, :request=> false}, :new_challenger => nil}
  ...
  defp reset_game_and_remove_CPU(state, game_state) do
    resp = Enum.map(state.players, fn x-> {elem(x,0), Warships.CPUPlayer.is_CPU_player?(state.game, elem(x, 0))} end)
  ...
  defp remove_a_player(state, name) do
    ...
    cond do
      ...
      true ->
        ...
        new_state = new_players |> Map.replace(:rematch, rematch_u )|> Map.replace(:state, :awaiting_players) |> Map.replace(:new_challenger, nil)
